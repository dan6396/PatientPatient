...
검색 품질은 감이 아니라 **골드 평가셋(3개 증례 · 77문항) 측정 기반**으로 튜닝했습니다.
...
**② 일반화 검증** (관절·복통·신경 3개 증례) — 한 증례로만 튜닝하면 다른 증례에서 OOD 누출(24%)이 생기는 **과적합을 평가셋 확장으로 발견** → 3증례로 게이트 재보정:

| 검증 | Recall | Hit@1 | OOD 누출 |
| --- | :---: | :---: | :---: |
| 3증례 재튜닝 | 92.9% | 85.7% | **0%** |
| **Leave-One-Out 교차검증** (2증례 튜닝 → 1증례 테스트) | **95.3%** | — | — |
