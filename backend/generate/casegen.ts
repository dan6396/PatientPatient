...
import { EXAM_PART_LABELS, ALL_EXAM_PARTS } from "../cases/exam-data";
...
반드시 아래 JSON "객체 하나"만 출력한다(마크다운/코드펜스 없이 순수 JSON):
{
  ...
  "examFindings": {
    "vital_signs": "<이 진단에서 나올 활력징후 소견>",
    "eyes": "<눈/결막 소견>",
    "mouth": "<구강 소견>",
    "lymph_nodes": "<림프절 촉진 소견>",
    "skin_nails": "<피부·손톱 소견>",
    "lung_auscultation": "<호흡음 청진 소견>",
    "joint_inspection": "<관절 시진 소견>",
    "joint_palpation": "<관절 압통·이개 촉진 소견>",
    "joint_rom": "<관절 운동범위(ROM) 소견>"
  }
}
gatedFacts는 병력청취 채점항목마다 1개 이상 만든다.
★ examFindings: 9개 신체진찰 파트 각각에 대해 "이 진단에서 실제로 나올 소견"을 한 줄로 구체적으로 쓴다(이 증례 진단에 맞게 — 통풍이면 통풍 소견, 골관절염이면 골관절염 소견). 그 진단과 무관해 정상인 파트는 "특이 소견 없습니다"로 둔다.`;
...
export function parseGeneratedCase(raw: string, id: string): PatientCase {
  ...
  // 신체진찰 소견(진단별) — examFindings(파트→소견 텍스트)를 CaseExamData로 변환
  let examData: CaseExamData | undefined;
  const ef = d.examFindings as Record<string, unknown> | undefined;
  if (ef) {
    const findings: Partial<Record<ExamPartId, CaseExamFinding>> = {};
    for (const partId of ALL_EXAM_PARTS) {
      const finding = asString(ef[partId]);
      if (!finding) continue;
      findings[partId] = {
        partId,
        label: EXAM_PART_LABELS[partId],
        finding,
        animationKey: `anim_${partId}`,
      };
    }
    if (Object.keys(findings).length) examData = { caseId: id, findings };
  }
